...
router.post("/jobs", jwtAuth, (req, res) => {
  ...
  if (user.type != "recruiter") {
    res.status(401).json({
      message: "You don't have permissions to add projects",
    });
    return;
  }
  ...
  job
    .save()
    .then(() => {
      res.json({ message: "Project added successfully to the database" });
    })
    .catch((err) => {
...
router.put("/jobs/:id", jwtAuth, (req, res) => {
  ...
  if (user.type != "recruiter") {
    res.status(401).json({
      message: "You don't have permissions to change the project details",
    });
    return;
  }
  Job.findOne({
...
router.delete("/jobs/:id", jwtAuth, (req, res) => {
  ...
  if (user.type != "recruiter") {
    res.status(401).json({
      message: "You don't have permissions to delete the project",
    });
    return;
  }
...
router.post("/jobs/:id/applications", jwtAuth, (req, res) => {
  ...
  if (user.type != "applicant") {
    res.status(401).json({
      message: "You don't have permissions to apply for a project",
    });
    return;
  }
  const data = req.body;
                    ...
                    if (myActiveApplicationCount < 10) {
                      Application.countDocuments({
                        ...
                        if (acceptedJobs === 0) {
                          ...
                          res.status(400).json({
                            message:
                              "You already have an accepted project. Hence you cannot apply.",
                          });
                        }
                      });
                    } else {
...
router.get("/jobs/:id/applications", jwtAuth, (req, res) => {
  ...
  if (user.type != "recruiter") {
    res.status(401).json({
      message: "You don't have permissions to view project applications",
    });
    return;
  }
  const jobId = req.params.id;
...
router.put("/applications/:id", jwtAuth, (req, res) => {
  ...
  if (user.type === "recruiter") {
          ...
          Job.findOne({
            ...
            Application.countDocuments({
              ...
              if (activeApplicationCount < job.maxPositions) {
                ...
                res.status(400).json({
                  message: "All positions for this project are already filled",
                });
              }
            });
          });
    ...
    if (status === "cancelled") {
      ...
      res.status(401).json({
        message: "You don't have permissions to update project status",
      });
    }
  }
});
          ...
          Application.countDocuments({
            ...
            .then((acceptedApplicant) => {
              if (acceptedApplicant > 0) {
                        ...
                        Job.findOneAndUpdate(
                          ...
                          .then((foundJob) => {
                            if (foundJob === null) {
                              res.status(400).json({
                                message:
                                  "Error while updating project's average rating",
                              });
                              return;
                            }
                            res.json({
                ...
                res.status(400).json({
                  message:
                    "You haven't worked for this project. Hence you cannot give a rating.",
                });
              }
            })
            .catch((err) => {
                ...
                .then((result) => {
                  ...
                  Job.findOneAndUpdate(
                    ...
                    .then((foundJob) => {
                      if (foundJob === null) {
                        res.status(400).json({
                          message: "Error while updating project's average rating",
                        });
                        return;
                      }
                      res.json({
